...
Card::Card(int _faceValue){
    faceValue = _faceValue;
}

Card::Card(int _faceValue, bool _revealed){
    faceValue = _faceValue;
    revealed = _revealed;
}
...
vector<int> Card::GetHandValues(vector<Card> h){
    vector<int> values;
    int aces = 0;
    int sum = 0;
...
void Card::FlipCard(Card& c) {
    // cout << "Flipping: " << c.faceValue << endl;
    c.revealed = c.revealed ? false: true;
}

int Card::GetCardValue() const{ return faceValue; }
